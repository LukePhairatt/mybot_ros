...
class Arduino:
    ...
    def __init__(self, port="/dev/ttyACM0", baudrate=57600, timeout=1):
        # TODO using rosparam to set this pid_rate
 
        self.PID_RATE = rospy.get_param("~pid_rate", 20) 
        # However, Change PID_RATE as appropriate in Arduino as well to match this if necessary 
        ...
        self.interCharTimeout = timeout / 30. # Do not change this-> linked to recv read command loop timeout
    ...
    def connect(self):
        try:
            ...
            # I change to (3 secs) for my Arduino serial.
            time.sleep(3)
            test = self.get_baud()
    ...
    def recv(self, timeout=0.5):
        ...
        while c != '\r':
            c = self.port.read(1)
            #print "Return char in recv cmd...", c #lk
            value += c
            attempts += 1
            if attempts * self.interCharTimeout > timeout:
               #print "shit happen return None...!" #lk
               return None
    ...
    def execute(self, cmd):
        ...
        try:
            self.port.write(cmd + '\r')
            value = self.recv(self.timeout)
            #print "Return value in recv cmd...", value #lk
