...
#(lk)#
To set SERVO_1 (servo[0] = pin 10, servo[1] = pin 9) to 30 degrees:

s 0 30

to read SERVO_1 angle:
t 0 
...
(lk)$ rosservice call /arduino/servo_write 0 120*pi/180

where id is the index of the servo as defined in the Arduino sketch (servos.h) and pos is the position in degrees (0 - 180).

#####
(lk) correction position is in radians !!! not in degrees (see arduino_driver.py -> servo_write(self, id, pos) for details (lk)
#####
**servo\_read** - read the position of a servo

    $ rosservice call /arduino/servo_read id
(lk)$ rosservice call /arduino/servo_read 0
